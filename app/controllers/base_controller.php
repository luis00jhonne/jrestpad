...
class BaseController {
	...
	public $doctype; /* Determina o DOCTYPE da página */
	/**
	 * Single instance of Controller @var BaseController
	 */
	 ...
	 *
	 * @param array $params
	 *        	Params(from url and HTTP requests) from view
	 * @param string $action
	 *        	Action to be called in controller
	 * @param mixed $app
	 *        	The object responsible to handle routes and render views(Actually Slim Framework app)
	 ...
	 *        	name (used to find the folder view folder)
	 * @param $action Controller
	 *        	action name (used to find the file basename)
	 * @param $params Params
	 *        	that will sended to view in $params array
	 * @param $layout Layout
	 *        	filename
	 */
	public function render($path, $params = array(), $layout = true) {
		...
		if (isset ( $controller ) && ! empty ( $controller )) {
			...
			$this->app->render ( $this->app->config ( "layout.file" ), array (
